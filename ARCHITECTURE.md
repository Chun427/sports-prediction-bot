...
GitHub Actions (cron */5, concurrency-serialized)
   ...
   addon overlays（guarded，不污染 core）
        |-- daily_report   （每日戰報）
        |-- awards_push    （FIFA-only 獎項；market-implied、N/A-safe、success-gated）
...
- **Awards push（FIFA-only 獎項）**：market-implied 機率（無盤 N/A）；**送成功才 mark**，全 N/A 仍 mark（避免重抓 API），明確失敗不 mark -> 下一 tick 重送。awards-YYYYMMDD 冪等。

## Never-Miss Recovery Flow

所有推播共用同一可靠性模型（success-gated retry queue）：

```
send()
  ├─ 成功      -> mark_pushed（pre/early/post/daily/awards/alert）-> 完成
  └─ 失敗      -> 不 mark -> 下一個 tick 重送
     (timeout / HTTP500 / network / 例外 / ok=False)   └─ 直到成功，或超過該推播 recovery boundary
boundary 後仍未成功 -> push_reconcile 發 admin 告警（最後一道保險）
```

- **Recovery boundary**：early 至 40m 交棒；pre 至 kickoff；post 至 PENDING_STALE_HOURS（約 36h）；daily 當日內；awards 當日內。
- **Partial failure 隔離**：逐場 try/except + continue，單場失敗不影響其他場。
- **不重複**：成功後以 flags（is_pushed）控制，postgame 另以 remove_prediction 清理；GitHub Actions concurrency 序列化防並發雙送。
- **可診斷**：postgame 各分支有 obs（no_result / not_completed / verify_none / evict_stale），漏推原因可從 Actions log 判讀。
...
2. **Postgame 為 event-driven，依賴賽果 API**：賽後推播在「賽果 API 回報 completed=true 且有分數」的那個 tick 才送，成功才 mark、失敗下一 tick 重送（Never-Miss）。但若 API 在 PENDING_STALE_HOURS（約 36h）內始終未回報該場（r=None／completed=false／缺分數），該場會過期驅逐而不推 —— 此為上游資料限制，非推播邏輯缺陷。已加 obs 診斷：`postgame.no_result` / `postgame.not_completed` / `postgame.verify_none` / `postgame.evict_stale`，可從 Actions log 判斷漏推原因。
